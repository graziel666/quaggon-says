...
void startGame(){
  ...
  while (nivelActual <= nivelMax){
    for (int i = 0; i <= nivelActual; i++){
      secuencia[i] =0; //random(4)%100;

      //down
      if (secuencia[i] == 0){
        unsigned long clockDelay = 1000;
        for (uint32_t tStart = millis();(millis()-tStart) < clockDelay){
          //targetTime = (currentTime + clockDelay); 
          //draws quaggon
          firstQuaggonDanceFrame = 0;
          lastQuaggonDanceFrame = 2;
          drawQuaggon();
          dispLives();
        ...
        unsigned long clockDelay2 = 300;
        for (uint32_t tStart = millis();(millis()-tStart) < clockDelay2){
